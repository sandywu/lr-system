...
if(pageSize > 5) {	//总页数 > 5
	if(thisPage < 4) {	//当前页为前三页
		for(var i=5;i>1;i--) {
			var txt = "<li><a href="+ (url+i) +">"+ i +"</a></li>";
			$('#first').after(txt);
		}
	} else {	//当前页大于三页
...
if(thisPage == 1) {			//当前为第一页a_Previous
	...
	$('#a_Previous').attr("href",url+(parseInt(thisPage)));
	$('#first').addClass("active");
	$('#a_Next').attr("href",url+(parseInt(thisPage)+1));
}
...
if(thisPage == pageSize) {	//当前页为最后一个
	...
	$('#a_Previous').attr("href",url+(parseInt(thisPage)-1));
	$('#a_Next').attr("href",url+(parseInt(thisPage)));
	activePage();
}
