...
public class InitController {
	...
	public String initIndex(HttpSession session, Model model) {
		...
		Collections.sort(news); 
		//查找为过期的系统消息的个数
		model.addAttribute("stmNumber",stmMassageService.findAllNumberByState(1).size());
	 ...
	 * @return
	 */
	@RequestMapping(value = "user/me/showStmMsg", method = RequestMethod.GET)
	public String showStmMsg( Model model) {
		model.addAttribute("stmMsgs", stmMassageService.findAllNumberByState(1));
		return "user/stmMsg/showStmMsg";
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "admin/mannageUser", method = RequestMethod.GET)
	public String initManageUser(@RequestParam("thisPage")int thisPage, Model model) {
		System.out.println("我是初始化mannageUser的控制层=============================");
		//1。查找所用用户的数量
		
		int pageSize = userService.findAllNumber()%pageNumber == 0? userService.findAllNumber()/pageNumber : userService.findAllNumber()/pageNumber+1;
		model.addAttribute("pageSize", pageSize);
		model.addAttribute("thisPage", thisPage);
		Page page = new Page((thisPage-1)*pageNumber, pageNumber);
		//2.查找前十条用户的详细信息
		model.addAttribute("users", userService.findByIndexPage(page));

		return "admin/mannageUser";
	}
	
	@RequestMapping(value = "admin/manageLog", method = RequestMethod.GET)
	public String initManageLog(@RequestParam("thisPage")int thisPage, Model model) {
		System.out.println("我是初始化manageLog的控制层=============================");
		//1。查找所用日记的数量
		int pageSize = logService.findAllNumber()%pageNumber == 0? logService.findAllNumber()/pageNumber : logService.findAllNumber()/pageNumber+1;
		model.addAttribute("pageSize", pageSize);
		model.addAttribute("thisPage", thisPage);
		Page page = new Page((thisPage-1)*pageNumber, pageNumber);
		model.addAttribute("logs", logService.findByIndexPage(page));

		return "admin/manageLog";
	}
	
	@RequestMapping(value = "admin/manageTODO", method = RequestMethod.GET)
	public String initManageTODO(@RequestParam("thisPage")int thisPage, Model model) {
		System.out.println("我是初始化manageTODO的控制层=============================");
		//1。查找所用TODO的数量
		int pageSize = todoService.findAllNumber()%pageNumber == 0? todoService.findAllNumber()/pageNumber : todoService.findAllNumber()/pageNumber+1;
		model.addAttribute("pageSize", pageSize);
		model.addAttribute("thisPage", thisPage);
		Page page = new Page((thisPage-1)*pageNumber, pageNumber);
		model.addAttribute("todos", todoService.findByIndexPage(page));

		return "admin/manageTODO";
	}
	
	@RequestMapping(value = "admin/manageSaid", method = RequestMethod.GET)
	public String initManageSaid(@RequestParam("thisPage")int thisPage, Model model) {
		System.out.println("我是初始化manageSaid的控制层=============================");
		//1。查找所用说说的数量
		int pageSize = saidService.findAllNumber()%pageNumber == 0? saidService.findAllNumber()/pageNumber : saidService.findAllNumber()/pageNumber+1;
		model.addAttribute("pageSize", pageSize);
		model.addAttribute("thisPage", thisPage);
		TodoPage page = new TodoPage((thisPage-1)*pageNumber, pageNumber);
		model.addAttribute("saids", saidService.findAll(page));

		return "admin/manageSaid";
	}
	
	@RequestMapping(value = "admin/manageStmMsg", method = RequestMethod.GET)
	public String initManageStmMsg(@RequestParam("thisPage")int thisPage,Model model) {
		System.out.println("我是初始化manageStmMsg的控制层=============================");
		//1。查找所用系统消息的数量
		int pageSize = stmMassageService.findAllNumber()%pageNumber == 0? stmMassageService.findAllNumber()/pageNumber : stmMassageService.findAllNumber()/pageNumber+1;
		model.addAttribute("pageSize", pageSize);
		model.addAttribute("thisPage", thisPage);
		Page page = new Page((thisPage-1)*pageNumber, pageNumber);
		model.addAttribute("stmMsgs", stmMassageService.findNumbByPage(page));

		return "admin/manageStmMsg";
	}
}
