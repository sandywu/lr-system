...
public class LogController {
	...
	private static final int addLogIntegral = 2;//添加日志得2积分
	private static final int delLogIntegral = -2;//删除日志扣2积分	
	@Autowired
	...
	public String addLog(@RequestParam("blogName")String blogName,@RequestParam("mood")String mood,
			@RequestParam("weather")String weather,@RequestParam("place")String place,
			@RequestParam("title")String title,@RequestParam("content")String content,
			HttpServletRequest req,RedirectAttributes attr) {
		System.out.println("====添加日志的标题==="+title);
		...
		Log log = new Log(whoCreate,createTime,title,blogName,mood,weather,place,content);
		log.setWhoCreate(whoCreate);
		
		if(req.getParameter("visible") == null) {	//所有人可见
			log.setVisible(true);
		} else {		//仅自己可见
			log.setVisible(false);
		}
		if(req.getParameter("reply") == null) {		//可回复
			log.setReply(true);
		} else {	//不可回复
			log.setReply(false);
		}
	...
	public String myLog(@RequestParam("thisPage")int thisPage,Model model, HttpSession session) {
		System.out.println("我是初始化我的日志页面的=============页数"+thisPage);
	...
	public String friendsLog(@RequestParam("thisPage")int thisPage,Model model, HttpSession session) {
		System.out.println("我是初始化所有日志页面的=============页数"+thisPage);
	...
	public String allLog(@RequestParam("thisPage")int thisPage,
		...
		System.out.println("我是初始化好友日志页面的====总条数："+count+"=========当前页"+thisPage+"，总页数："+pageSize);
	...
	public String delLogByIds(@RequestParam("logId")int id,
			...
			@RequestParam("whoCreate")String whoCreate,
			RedirectAttributes attr) {
		System.out.println("===---=删除log的id是："+id+"，当前页数是："+thisPage+",日志本："+blogName+"，创建人："+whoCreate);
		String message = "";
		if(logService.delLogById(id)==1) {
