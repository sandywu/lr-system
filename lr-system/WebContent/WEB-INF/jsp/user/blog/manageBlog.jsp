		...
		<div class="row edit_cont">
			...
			<form action="addBlog" method="get">
				<span class="row"> <input name="name" type="text"
					...
					autofocus="autofocus" placeholder="日记本名..." /> <small
			...
			<h4>
			已有日记本<small>( <span>${fn:length(blogs)}</span> ) 
			->( <span>默认日记本不可删除，删除的日记本下所有日记将会自动转移到默认日记本下</span> )
				</small>
			</h4>
			<br />
			<c:choose>
					...
					<form action="delBlog" method="get">
						...
						<select name="name" class="form-control" id="addlog_sel"
							...
							<c:forEach items="${blogs}" var="blog">
								<c:choose>
									<c:when test="${blog.name eq '默认日记'}"></c:when>
									<c:otherwise>
