...
<body style="border: 0px; padding: 0px; height: 1200px;width: 950px;">
<c:choose>
	...
	<c:forEach items="${saids}" var="said">
		<div class="row" style="margin-top: 25px; padding-left: 20px;">
			...
			<c:choose>
				<c:when test="${said.whoCreate==sessionScope.user.nickName}">
					<a class="user"
				 	name="../me/myself"
				...
				<c:otherwise>
				<a class="user"
				 	name="../me/friendsIndex?whoCreate=${said.whoCreate}"
					onclick="return a(this)">
				</c:otherwise>
			</c:choose>
			
			<c:choose>
					<c:when test="${said.userPhoto == null}">
						<img src="../../photo/system.jpg" width="50px" height="50px"
						 title="${said.whoCreate}" alt="用户头像"/>
					</c:when>
					<c:otherwise>
					 <img src="../../photo/${said.userPhoto}" width="50px" 
			...
			<div style="margin-left:60px;">
				<label>${said.whoCreate} 说：</label>
				<c:choose>
				<c:when test="${said.whoCreate==sessionScope.user.nickName}">
					<a href="delSaid?saidId=${said.id}&thisPage=${thisPage}"
			...
			<div style="margin-left: 60px;">
				...
				<a name="${sessionScope.user.id}=${said.id}" onclick="allSaidpraise(this)" title="说的很好，赞一个">
					<label> 
	...
	<nav class="fenye">
		<ul id="pagination" class="pagination">
			...
			<li id="first"><a href="./../friends/allSaid?thisPage=1">1</a></li>
	...
	<script src="./../../js/allSaid.js" type="text/javascript"></script>
